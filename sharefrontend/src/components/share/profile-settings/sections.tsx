  ...
  return (
    <div className="flex flex-col gap-4 px-5 py-5 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex items-center gap-4">
        <div className={`dream-chip flex h-14 w-14 items-center justify-center ${muted ? "bg-[#f6d4e4] text-[var(--brand-strong)]" : "bg-[var(--button-primary)] text-[var(--foreground)]"}`}>
          {icon}
        </div>
        <div>
      ...
      <button type="button" onClick={onClick} className="btn-subtle self-end rounded-full px-5 py-2.5 text-sm font-black text-[var(--foreground)]/76 sm:self-auto">
        {buttonLabel}
      </button>
    </div>
  );
...
export function RoleChip({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`rounded-full px-4 py-1.5 text-xs font-black transition ${active ? "btn-primary text-[var(--foreground)]" : "btn-subtle text-[var(--foreground)]/72"} disabled:cursor-not-allowed disabled:opacity-60`}
    >
      {children}
    </button>
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(38,24,27,0.18)] p-4 backdrop-blur-sm sm:p-6">
      ...
      <div className="dream-panel relative z-10 w-full max-w-[500px] p-6 sm:p-7">
        <div className="flex items-start justify-between gap-4">
          ...
          <button type="button" onClick={onClose} className="btn-subtle flex h-11 w-11 items-center justify-center rounded-full text-[var(--foreground)]/62" aria-label="关闭">
