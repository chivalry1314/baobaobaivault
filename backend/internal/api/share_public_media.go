...
func (h *Handler) shareDiscoverCards(c *gin.Context) {
	...
	cards, total, err := h.shareService.ListDiscoverCards(c.Request.Context(), page, size, viewerUserID, slot)
	if err != nil {
...
func (h *Handler) shareDiscoverDesktopComponents(c *gin.Context) {
	page := 1
	if value := strings.TrimSpace(c.Query("page")); value != "" {
		parsed, parseErr := strconv.Atoi(value)
		if parseErr != nil || parsed <= 0 {
	...
	items, total, err := h.shareService.ListDiscoverDesktopComponents(c.Request.Context(), page, size)
	if err != nil {
		jsonError(c, http.StatusInternalServerError, err)
		return
	}
