...
func (h *Handler) registerShareRoutes(r *gin.Engine) {
	group := r.Group("/api/share")
	{
		...
		group.GET("/discover/category-settings", h.sharePublicCategorySettings)
		...
		me.Use(h.shareRequireAuth())
		{
			...
			me.GET("/favorites", h.shareMyFavorites)
			me.POST("/favorites/:cardId", h.shareFavoriteCard)
			me.DELETE("/favorites/:cardId", h.shareUnfavoriteCard)
			me.GET("/access-codes", h.shareMyAccessCodes)
			me.POST("/cards", h.shareCreateCard)
			me.POST("/admin/cards", h.shareCreateCardBundle)
			me.POST("/admin/cards/presign", h.shareCreateCardBundlePresign)
			me.POST("/admin/cards/complete", h.shareCreateCardBundleComplete)
			...
			me.GET("/system/category-settings", h.shareSystemCategorySettings)
			me.PATCH("/system/category-settings", h.shareSystemUpdateCategorySettings)
			...
			me.POST("/cards/:cardId/cover/presign", h.shareReplaceCardCoverPresign)
			me.POST("/cards/:cardId/cover/complete", h.shareReplaceCardCoverComplete)
			me.DELETE("/cards/:cardId/cover", h.shareDeleteCardCover)
			me.PUT("/cards/:cardId/assets/:slot", h.shareReplaceCardAsset)
			me.POST("/cards/:cardId/assets/:slot/presign", h.shareReplaceCardAssetPresign)
			me.POST("/cards/:cardId/assets/:slot/complete", h.shareReplaceCardAssetComplete)
